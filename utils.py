...
def clean_site_names_from_title(title):
    ...
    patterns_to_remove = [
        ...
        r'\s*[-|~•]\s*Electro.*$',
        r'\s*[-|~•]\s*Info.*$',
        r'\s*à\s+(Linksolutions?|Crenova|Duga|Tabtel|Maroc).*$',  # Plus spécifique: seulement "à [nom de site]"
...
def update_product_in_database(product_id, data, site_name, scraping_success=True):
    """
    Met à jour le produit dans la base de données
    
    Args:
        product_id: ID du produit
        data: Données scrapées (peut être vide ou None si échec)
        site_name: Nom du site (crenova, tabtel, etc.)
        scraping_success: True si scraping réussi, False si échec
    """
    try:
        conn = connect_scraper()
        cursor = conn.cursor()
        ...
        if scraping_success and data:
            ...
            check_query = f"SELECT id FROM ps_products_comparison_v2 WHERE id = %s"
            cursor.execute(check_query, (product_id,))
            if not cursor.fetchone():
                print(f"❌ Produit {product_id} non trouvé dans la base")
                cursor.close()
                conn.close()
                return False
            ...
            # CAS: Scraping échoué - on met juste à jour la date et le statut
            # CORRECTION: Vérifier d'abord si le produit existe
            check_query = f"SELECT id FROM ps_products_comparison_V2 WHERE id = %s"
            cursor.execute(check_query, (product_id,))
            if not cursor.fetchone():
            ...
            query = f"""
            UPDATE ps_products_comparison_V2 
            SET {site_name}_scraped_at = NOW(),
                {site_name}_status = 1  -- ⭐ Important: on met à 1 même si échec
            WHERE id = %s
            """
            ...
            if rows_affected > 0:
                print(f"⚠️  Produit {product_id} - Scraping échoué, statut mis à 1")
                print(f"   (Ne sera pas retenté)")
            else:
                print(f"⚠️  Aucune ligne mise à jour pour le produit {product_id}")
